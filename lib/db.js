...
var rPushToCache = function(key, value, callback) {
  cache.checkClient(function(err, client) {
    if (err) {
      return callback(vlog.ee(err, 'pushToCache:checkClient:' + key + ',' + JSON.stringify(value)));
    }
    // var value = JSON.stringify(obj);
    client.rpush(key, value, function(err, re) {
      if (err) {
        return callback(vlog.ee(err, 'pushToCache:rpush:' + key + ',' + value));
      }
      return callback(null, re);
    });
...
exports.rPushToCache = rPushToCache;
